...
router.post('/firebase-google', async (req, res) => {
    ...
    try {
        const decodedToken = await (0, auth_2.getAuth)(firebaseAdmin_1.firebaseApp).verifyIdToken(idToken);
        const email = decodedToken.email;
        const firebaseUid = decodedToken.uid;
        const name = decodedToken.name || 'Firebase User';
        const avatar = decodedToken.picture || null;
        if (!email)
            return res.status(401).json({ message: 'Invalid Firebase token (no email)' });
        let user = await db_1.default.user.findFirst({ where: { OR: [{ email }, { googleId: firebaseUid }] } });
        if (!user) {
            user = await db_1.default.user.create({ data: { googleId: firebaseUid, email, name, avatar } });
        }
        else if (!user.googleId) {
            user = await db_1.default.user.update({ where: { id: user.id }, data: { googleId: firebaseUid, avatar: avatar || user.avatar } });
        }
        const token = signToken(user.id);
        const { passwordHash: _, ...safeUser } = user;
        res.json({ token, user: safeUser });
...
router.post('/link-phone', auth_1.authenticate, async (req, res) => {
    const { idToken } = req.body;
    if (!idToken) {
        return res.status(400).json({ message: 'Firebase ID token is required' });
    }
    try {
        ...
        const existing = await db_1.default.user.findFirst({ where: { phone, NOT: { id: req.userId } } });
        if (existing) {
